...
fixed upstream). It grew into **shipping fixes and features** through pull requests.
...
- **A payment-link request flow, end to end.** From a stakeholder request to shipped
  ...
  to the top of the advisor dashboard, since a student asking to pay is time-sensitive.
  The bot deliberately never sends the link itself: links are personalized and identity
...
- **Serverless state design.** How a stateless Worker keeps per-student conversation
  ...
  completed conversations to R2, and surfaces structured cases from D1, plus the
